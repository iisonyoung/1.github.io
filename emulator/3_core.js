...
Object.values(UI.overlays).forEach(overlay => {
    ...
    // --- Global Keyboard & Input Manager for PWA ---
    // Handles soft keyboard appearing and squishing the viewport on mobile devices (interactive-widget=resizes-content)
    const setupGlobalInputManager = () => {
        ...
        // Try to handle when focus happens
        document.addEventListener('focusin', (e) => {
            const tagName = e.target.tagName?.toLowerCase();
            if (tagName === 'input' || tagName === 'textarea' || e.target.isContentEditable) {
                // Focus triggered, we expect visualViewport resize next
                
                // If it's a specific chat input, we might want to manually scroll its container
                // But we let resize handle the main pushing
            }
        });
            ...
            if (tagName === 'input' || tagName === 'textarea' || e.target.isContentEditable) {
                ...
                setTimeout(() => {
                    if (!document.activeElement || (document.activeElement.tagName !== 'INPUT' && document.activeElement.tagName !== 'TEXTAREA' && !document.activeElement.isContentEditable)) {
                        ...
                        // DO NOT OVERRIDE BODY HEIGHT: let CSS handle top:0, bottom:0 for PWA 100% full screen
        ...
        window.visualViewport.addEventListener('resize', () => {
            ...
            const currentOffset = vv.offsetTop;
            
            // If the viewport shrunk significantly, keyboard is likely open
            if (currentHeight < originalHeight * 0.85) {
                isKeyboardOpen = true;
                // DO NOT OVERRIDE BODY HEIGHT: let CSS handle top:0, bottom:0 for PWA 100% full screen
                // body.style.height = `${currentHeight}px`;
                
                // If an element is focused, ensure it stays visible
                if (document.activeElement && typeof document.activeElement.scrollIntoView === 'function') {
                    // Small delay to let browser do its own thing first
                    setTimeout(() => {
                        document.activeElement.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
                    }, 50);
                }

                // If we are in a chat view, scroll the messages down
                // Look for active chat container
                const activeViews = document.querySelectorAll('.app-view.active .ins-chat-messages, .tk-chat-scrollable, #tk-dm-messages-container, #yt-player-chat-container, #yt-user-live-chat-container, .yt-bubble-chat-container');
                activeViews.forEach(container => {
                    if (container && container.offsetParent !== null) { // if visible
                ...
                originalHeight = window.innerHeight; // Update original in case of rotation
            }
        });
        
        // Also prevent pull-to-refresh on input areas
        document.addEventListener('touchmove', function(e) {
            if (document.activeElement && (document.activeElement.tagName === 'INPUT' || document.activeElement.tagName === 'TEXTAREA')) {
                // If we are at the top of a scroll container, don't let it pull to refresh
                const scrollable = e.target.closest('.ins-chat-messages, .tk-chat-scrollable, .yt-bubble-chat-container');
                if (!scrollable || (scrollable.scrollTop === 0 && e.touches[0].clientY > (scrollable.lastY || 0))) {
                    // It's trying to scroll up past the top. Let's just not preventDefault always as it breaks scrolling.
                    // Actually, interactive-widget mostly solves this. We will rely on CSS overscroll-behavior: none.
                }
                if(scrollable) scrollable.lastY = e.touches[0].clientY;
            }
        }, { passive: true });
    };

    setupGlobalInputManager();
});
    }
});
...
function initMobileKeyboardFixes() {
    ...
    const updateKeyboard = () => {
        ...
        // 在 standalone 模式下，高度计算误差可能较大，如果不大于 150，重置为 0。
        if (keyboardHeight < 120 || (isStandalone && keyboardHeight < 150)) {
            keyboardHeight = 0;
        }
...
// ==========================================
const homeScreen = document.getElementById('app');
