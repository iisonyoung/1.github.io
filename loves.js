...
3. 例如：["写得真好！", "下次一起去啊。"]`;
    ...
    showFriendDetail: function(friend) {
        const detailArea = document.getElementById('loves-detail-area');
        const detailName = document.getElementById('loves-detail-name');
        const detailAvatarImg = document.getElementById('loves-detail-avatar-img');
        const detailAvatarIcon = document.getElementById('loves-detail-avatar-icon');
        const inviteBtn = document.getElementById('loves-invite-btn');
        const enterBtn = document.getElementById('loves-enter-btn');
        
        if (!detailArea) return;
        
        detailArea.style.display = 'flex';
        detailName.textContent = friend.nickname || friend.realname || 'Unknown';
        
        if (friend.avatarUrl) {
            detailAvatarImg.src = friend.avatarUrl;
            detailAvatarImg.style.display = 'block';
            detailAvatarIcon.style.display = 'none';
        } else {
            detailAvatarImg.style.display = 'none';
            detailAvatarIcon.style.display = 'block';
        }
