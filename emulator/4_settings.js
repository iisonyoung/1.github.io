...
document.getElementById('format-all-data-btn')?.addEventListener('click', async () => {
    if (confirm('警告：此操作将执行彻底的格式化！\n清空所有缓存、应用数据和配置状态，系统将完全重置。此操作不可恢复！')) {
        try {
            // 清理 localStorage
            localStorage.clear();
            
            // 清理 IndexedDB
            if (window.appStorage && window.appStorage.clearAllPersistentData) {
                await window.appStorage.clearAllPersistentData();
            }
            if (window.appStorage && window.appStorage.clearAllData) {
                await window.appStorage.clearAllData();
            }

            showToast('系统已彻底格式化，正在重启...');
            setTimeout(() => location.reload(), 1500);
        } catch (err) {
            console.error('Failed to format all data', err);
            showToast('格式化失败，请手动清除浏览器缓存');
        }
    }
});
